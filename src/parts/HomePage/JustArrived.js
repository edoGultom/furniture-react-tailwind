    ...
    .map((_, index) => {
      return (
        <div className="px-4 relative card group" key={index}>
          <div
            className="rounded-xl bg-gray-300 overflow-hidden card-shadow relative loading"
            style={{ width: 287, height: 386 }}
          ></div>
          <div className="w-24 h-3 bg-gray-300 mt-3 rounded-full loading"></div>
          <div className="w-36 h-3 bg-gray-300 mt-2 rounded-full loading"></div>
        </div>
      );
    });
}
